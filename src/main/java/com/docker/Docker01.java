 ...
 * @Date 2018/11/2
 ...
 docker容器与虚拟机比对
    传统虚拟机技术是虚拟出一套硬件后（很重要），在其上运行一个完整操作系统，在该系统上再运行所需应用进行
 ...
 查看centos版本 cat /etc/reahat-release
 ...
 docker 常用命令
    ...
    docker run [option] image名字   从image创建一个容器实例，并启动它
        -d 守护进程的方式启动（运行在后台,没有前端交互窗口）
        -it 进入容器交互界面
    ...
    docker exec -it 容器id bin/bash    进入容器交互界面  退出交互界面用exit，容器不会停止 如果用attach ，退出后容器会停止
